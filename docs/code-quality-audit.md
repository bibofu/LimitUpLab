...
## 2026-09-13：旧 Agent 评测资产退役

- 范围：删除 Chat Eval V2 Dev、Live Behavioral Eval V1、两套冻结工具世界、V1 迁移清单、题库样本、旧基线和专属评测说明；历史审查记录不改写。
- 原因：旧集合仍以 Query/Planner/Tool Policy 或过渡期 Plan-and-Execute 为中心，答案事实关系断言不足，不能覆盖当前 bounded ReAct 的 Observation 后决策、集合证据 lineage、Requirement/finish 终态以及实体、日期、指标关系正确性。
- 运行边界：统一项目检查和系统健康不再加载旧数据集；普通 pytest 与前端构建仍保留，但不构成 Agent 行为质量或发布证明。旧 Evaluator/CLI 仅为后续重建设计暂存，不得生成新的现行质量结论。
