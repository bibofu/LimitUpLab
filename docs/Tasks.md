...
V1 最终定位：使用完整收盘数据，为最新交易日首板生成次日一进二 Top10 观察候选，并持续回填 D+1 至 D+5 结果。V1 可按需读取带时间戳的热股榜、行业强弱榜和东方财富、同花顺财经快讯，但不把它们用于评分；盘中、集合竞价、通用网页搜索和其他策略属于 V2。
