...
- `[x]` 完成 09:25 集合竞价终选 v1。
  - 前一晚初选快照保持不可变，竞价终选使用独立 SQLite 表按交易日和版本固化。
  - 同花顺 `auction-snapshot --stage final` 只在 `closed + final` 时进入评分，缺失或非终值不落库。
  - 一进二从上一交易日完整合格首板池重排；首板挖掘从每日 Top30 候选池终选 Top10，旧 Top10 快照显式降级为名单内重排。
  - 终选分数采用 80% 原评分与 20% 竞价确认，温和高开、竞价量比和竞价换手提供确认，临近一字、明显低开和量能不足记录扣分风险。
...
- `[x]` 完成两类盘前推荐的半小时情报刷新。
  - 首板挖掘 Top10 与一进二接力 Top10 共用独立情报快照，每 30 分钟批量更新最新价格、涨幅和成交额。
  - 每只候选同步最近 7 日个股新闻和最新季度利润表，计算同报告期营收与归母净利润同比；财报按 24 小时复用。
  - 原始推荐分与预测快照保持不可变，动态情报单独存储，避免污染真实预测回测。
  - 前端每 30 分钟自动读取最新快照，展示刷新时间、最新行情、新闻标题和财报摘要。
