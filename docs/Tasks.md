...
V1 最终定位：使用完整收盘数据，为最新交易日首板生成次日一进二 Top10 观察候选，并持续回填 D+1 至 D+5 结果。V1 可按需读取带时间戳的热股榜和东方财富、同花顺财经快讯，但不把它们用于评分；盘中、集合竞价、实时板块、通用网页搜索和其他策略属于 V2。
...
- `[x]` 增加 V1 范围自动评测。
  - 覆盖实时板块、盘中价格、集合竞价、公司最新新闻和通用网页搜索等越界问题，并验证热股榜、综合财经快讯可基于结构化来源回答。
