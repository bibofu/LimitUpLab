...
## 涨停后策略平台双核演进（2026-09-07）

- `[x]` 下线旧实验链路。
  - 清理前使用现有备份能力生成时间戳 SQLite 备份。
  - 删除旧 API、服务、仓库、模型、脚本、测试、Agent 工具、提示词和前端入口。
  - 删除旧快照表及索引，清空混合 recommendation intelligence 临时/current/change/final 记录。
  - 校验一进二不可变预测、Outcome、复盘快照和评分策略记录数量不变。

- `[x]` 建立统一策略注册表。
  - 固定顺序注册一进二接力、高位回撤、横盘缩量、回撤企稳、强势不连板、断板修复和二进三。
  - 每项声明版本、生命周期、候选范围、涨停锚点、截止契约、成熟度、输出类型、所需数据、缺失处理、Outcome 与晋级门槛。
  - 一进二为前向验证研究排名；其余为探索观察池。

- `[x]` 建立通用策略快照与 Outcome。
  - 新增策略运行、候选和 Outcome 表，首次结果不可变。
  - 候选必须具有不晚于信号日的真实涨停锚点。
  - D+1、D+3、D+5 使用精确市场交易日，缺失不顺延。
  - 日更流水线生成七项策略快照并回填成熟结果。

- `[x]` 完成公共接口和前端策略列表。
  - 提供目录、最新结果、历史、单股路径和统计五组 API。
  - 前端按固定顺序展示阶段、成熟度、数据日、样本和输出类型。
  - 仅一进二展示排名；探索策略只展示无名次观察池。
  - 策略详情统一为候选、单股路径、历史统计和方法说明。

- `[x]` 完成 Agent 统一入口。
  - 增加策略目录、注册策略最新结果、策略内单股路径和多策略统计工具。
  - 策略回答携带版本、截止日、成熟度、输出性质和涨停锚点。
  - 比较同时展示样本量、完整度、时间范围和风险指标；样本不可比时拒绝优劣结论。
  - Agent 不临时创造评分、不修改权重、不登记 Challenger。

- `[x]` 完成分阶段提交和回归。
  - 后端策略/清理、前端工作台、Agent 统一入口和文档分别提交。
  - 运行后端定向与完整确定性测试、Agent Core/Product/Query Contract Eval、前端测试和生产构建。
...
- `[x]` 实现首板候选池过滤。
  ...
  - 排除北交所股票。
  - 排除科创板股票。
  - 一进二接力动态候选、每日 Top10 固化预测与盘前接力名单排除创业板，不影响涨停池和个股详情。
  - 排除新股 / 次新股。
  - 排除成交额过小样本。
...
- `[x]` 旧的混合候选刷新已退役；推荐情报流水线收敛为 relay-only。
