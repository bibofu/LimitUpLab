...
- `[x]` 完成首板挖掘 v1 量价基线。
  ...
  - 通过同花顺全市场行情分页扫描约 5500 只股票，排除 ST、退市风险、新股标记、北交所、科创板、当日已涨停和成交额不足样本。
  - 第一阶段按当日涨幅、收盘位置、振幅、实体和成交额召回 Top60；第二阶段并发补齐近 60 日 K 线后精排 Top10。
  - 评分覆盖短期动量、收盘强度、量能扩张、突破位置、流动性、日内质量和数据完整性，并输出位置形态、置信度、理由与风险。
...
- `[~]` 增强首板挖掘输入与结果闭环。
  - v1 暂未将流通市值、板块强度、公告和新闻催化纳入分数，页面与回答必须明确这是量价结构基线，不得表述为涨停概率。
  - 下一版补充市值偏好、行业相对强度和事件催化，同时保持每项事实的时点可用性与缺失标记。
