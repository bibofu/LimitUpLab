...
- `[x]` 完成用户登录系统 MVP。
  - 增加统一用户、外部身份、登录 Session 和一次性认证挑战表。
  - 支持 GitHub OAuth Web Flow，使用 `state + PKCE`，以 GitHub 稳定数字 ID 识别用户，不持久化 Access Token。
  - 支持邮箱验证码登录，验证码限时、限频、限制尝试次数，数据库只保存 HMAC 摘要。
  - 登录 Session 使用高熵随机 Token，数据库只保存 SHA-256 摘要，浏览器使用 HttpOnly、SameSite Cookie。
  - 首次登录将当前匿名聊天会话原子迁移到用户账号；公开行情保持匿名可读，生产环境 Agent 对话要求登录。
  - 前端增加登录弹窗、GitHub/邮箱入口、用户状态和退出功能。
