...
这些结果来自不同样本和代码时点，不能合并成统一成功率。重构前必须重新冻结同版本、同模型、同数据、同预算的基线；不能仅凭少量页面试验或合成测试通过宣称新架构更优。
...
模型请求建议最多 8 次，失败也计数；开始新一轮前保留一次收尾机会；同参数瞬时错误最多重试一次，回答修复最多一次。deadline 和 token 上限在基线阶段确定并配置化，不能写死未经验证的延迟承诺。
...
1. 固定代码版本、模型标识/配置、fixture、seed、问题集和预算，保留原始trace。
2. 盘点全部工具的输入输出、历史支持、错误状态、隐藏调用与成本。
3. 验证现用Provider的多轮tool calling、多个调用ID、工具错误回传、usage、超时和取消支持。
4. 确定初始deadline/token预算与不支持能力清单。

交付：可复跑基线、工具覆盖表、模型兼容测试。实际缺数据与规划错误必须分开归因。

### B. 最小纵向 ReAct 闭环

1. 接通标准消息模型适配与最小StateGraph。
2. 实现共享Policy、逐调用结果、Observe、统一预算和顶层终态。
3. 先接名单、新闻、K线、实体解析及必要计算工具，覆盖简单、动态名单、条件分支、单项失败。
4. 对相同问题运行冻结工具世界下的真实模型对照。

门槛：四类场景走通真实调用；没有关键词complex路由、独立Replanner或原始JSON答案。若质量未改善，停在此阶段分析，不扩大迁移掩盖问题。

### C. 全工具与多轮接入

1. 按覆盖表逐项接通全部既有研究工具；先验证契约再注册。
...
1. 完成冻结Live A/B、稳定性、Holdout和真实HTTP/页面业务验收。
