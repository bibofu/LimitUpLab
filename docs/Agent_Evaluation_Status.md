...
- 本批已有6题通过用户业务审核：OFF-001、OFF-010、OFF-027、OFF-028、OFF-031、OFF-032。
  用户逐题明确认可口径、标准事实与判分规则；确认绑定当前suite v3 / case v5及各自Case/World摘要，
  保存在 `output/agent-eval/datasets/local30-v4/user-review-20260914-batch01.json`。
  原suite.json/审核包保留生成时快照，此后审批以该独立记录为准，不覆盖旧审批与运行结果。
  其余24题未获本次批准；这6题仍需相应技术/抽取校准验收，不因此自动晋升Active。
- 30题当前仍为Golden候选（6题业务已确认、24题待业务审核），不是30道已激活Golden，也不是28/30完整质量通过。
