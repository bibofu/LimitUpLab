...
complete/needs_review；扩展670 passed、10 skipped。但手写名单绕开table及无据字段补全仍存在（BC-086），不能算质量通过。
下一步先裁决残留问题及例举与正式名单边界，再收敛通用交付约束；不再补fixture、不叠Q01专用提示或反复采样。
保留截断检查，不用问题正则或限字/条数补丁，不扩大评测框架。
其他4题不重跑挑答案；
完成实际答案人审（包括Q03来源措辞），再进入多轮执行。一次complete不等于稳定性通过，业务认可不等于答案认可。
