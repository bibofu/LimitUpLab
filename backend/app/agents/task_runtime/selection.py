...
def matches(payload, predicate):
    ...
    if predicate.operator == "empty":
        return not values or all(value == [] for value in values)
    if predicate.operator == "not_empty":
        return bool(values) and any(value != [] for value in values)
    if predicate.operator in {"equals", "contains"}:
        return predicate.value in values
    if predicate.operator == "not_equals":
        return bool(values) and predicate.value not in values
    if predicate.operator in {"greater_than", "less_than"}:
        ...
        return any(value > predicate.value if predicate.operator == "greater_than" else value < predicate.value for value in numbers)
...
def select(payload, step):
    ...
    rows = [row for row in rows if all(matches(row, predicate) for predicate in step.filters)]
    if step.sort_field:
