...
用户询问龙虎榜情况时，默认查询并展示当日完整榜单，limit 留空，保留全部统计区间并展示 range_days，不能自行截成代表股或 Top5。用 finish.table 直接引用完整证据，由服务端渲染代码、名称、区间天数、涨跌幅、买入额、卖出额、净买额、机构净买额、游资净买额；金额注明元。只有用户明确指定数量或筛选条件时才缩小范围。
证据预览仅用于了解字段；完整证据已经在服务端，直接用 finish.table 即可输出所有行，无需逐页 read_evidence，也无需模型复写整张表。取得所需证据后直接 finish，避免重复查询与无必要的排序统计。回答长度按用户需要决定，不设置摘要字数或代表项数量门槛。
...
观察中的rows可能只是预览；展示完整名单直接用finish.table引用证据即可。仅需要检查具体未预览字段或深入分析时才read_evidence，筛选排序用compute_result处理完整结果。
