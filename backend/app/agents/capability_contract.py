...
CAPABILITIES: tuple[AgentCapability, ...] = (
    ...
    AgentCapability(
        "limit_up_pool",
        "查询某个交易日的涨停、首板、连板或炸板名单，也可查询指定行业或题材近期"
        "涨停过的股票；不用于跨日晋级数量和比例。",
        ...
        answer_guidance=(
            ...
            "使用最新完整交易日并写明 ISO 日期；指定板块的近期名单默认覆盖最近 7 个"
            "交易日。先报告匹配数量，再按事实顺序列名称和代码。"
    ...
    AgentCapability(
        ...
        answer_guidance=(
            ...
            "相对涨停收盘变化、区间幅度和量比；缺失数据必须披露。"
        ),
    ),
    AgentCapability(
        "post_limit_path",
        "分析一只明确股票从某次收盘涨停开始的逐日价格、回撤和成交量路径。",
        (CapabilityToolRequirement("post_limit_path"),),
        examples=("这只票涨停后怎么走的", "001299涨停后的逐日走势", "它为什么算高位回撤"),
        answer_guidance=(
            "写明股票、涨停锚点和数据截止日，按交易日列出相对涨停收盘变化、"
            "相对运行高点回撤和相对涨停日成交量；只描述已发生路径。"
        ),
    ),
    AgentCapability(
        ...
        answer_guidance=(
            ...
            "同时报告样本数、信号日数、完整度、均值、中位数、正比例、MAE和MFE。"
            "样本质量不足时不得下形态优劣结论。"
        ),
    ),
    AgentCapability(
