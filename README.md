...
`v1.0.0` 是首个可用基线，定义为基于完整收盘数据的首板复盘与次日一进二候选研究工具。当前开发分支已开始 V2 增量：前一交易日收盘结果形成 Top30 候选草稿，新闻与财报持续重排，交易日 09:25:10 再结合集合竞价终值生成两类策略当天唯一正式 Top10，复盘只读取该正式版本。
...
4. V1 日期保存收盘 Top10；V2 启用日起只生成可覆盖的盘前候选草稿。
...
任务默认在每个工作日 `16:10` 执行，并启用错过后补跑、失败重试和禁止并发执行。一次执行会自动完成交易日判断、原始数据同步、首板特征与 enrichment、盘前候选草稿、历史 Top10 的 D+1 至 D+5 K 线缓存、Outcome 回填、每日复盘快照和健康检查。V2 启用日起，收盘任务不再把草稿标记为 `live`；次日 `09:25:10` 的竞价终选才保存正式预测并进入后续复盘。
