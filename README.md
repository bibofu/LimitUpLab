...
> 面向已经发生涨停的股票，以可审计策略引擎为底座、以 Agent 为统一交互入口的收盘后研究平台。

产品采用双核协同：

- 策略核按固定目录组织涨停后的筛选、排名、观察、不可变快照与 Outcome 复盘。
- Agent 核查询、筛选、解释、比较和复盘注册策略结果，不临时创造分数或修改策略。
- 所有展示候选必须带有不晚于信号日的真实涨停锚点、数据截止日、策略版本、证据与缺失项。

当前策略目录按固定顺序包含“一进二接力、高位回撤、横盘缩量、回撤企稳、强势不连板、断板修复、二进三”。一进二处于 `forward_validation + ranked_research`，保存不可变 Top10；其余策略处于 `exploratory + observation_pool`，保存无名次观察池，不展示预测概率、胜率承诺或跨策略总榜。

当前版本已经具备：

- 真实涨停、炸板、K 线、板块、人气和龙虎榜数据流水线。
- 统一策略注册表、通用策略运行/候选/Outcome 存储及 D+1、D+3、D+5 精确交易日对齐。
- 一进二首板 Facts、规则评分、不可变 Top10、前向追踪和 Champion/Challenger 审计。
- 六类涨停后探索观察策略，以及候选、路径、历史统计和方法说明四区详情。
- 五组公共策略 API 与按固定顺序展示的前端策略工作台。
- 原生 Function Calling Planner、Tool Policy、SSE 流式回答与策略目录/结果/单股/比较工具。
- 数据健康、Agent 运行轨迹、缓存、备份和离线回归评测。
- Docker、Nginx、Windows 日更任务与 SQLite 单机部署基线。

早期的集合竞价与扩展候选研究实验不再属于当前运行链路。历史版本仍由 Git 标签保存；当前数据库清理前会生成本地时间戳备份。

评分和观察结果仍处于研究与验证阶段。项目已形成数据与评价闭环，但不声称能够稳定预测后续表现。
...
策略注册表是产品和 Agent 的单一事实源。每项策略固定声明 `strategy_id`、名称、版本、生命周期阶段、候选范围、涨停锚点要求、数据截止契约、成熟度、输出类型、所需数据、缺失处理、Outcome 指标和晋级门槛。
...
排名策略保存不可变 Top10；观察策略保存不可变观察池但不生成名次。持续符合可以继续展示，首次信号不会被重复计数。Champion/Challenger 只作用于具备排名资格的策略。
...
| `strategy_catalog` | 查询固定策略目录、版本、成熟度、输出性质和最新覆盖 |
| `strategy_latest` | 读取注册策略的不可变最新排名或观察池 |
| `strategy_stock_path` | 查询策略内单股涨停锚点、入选证据和逐日路径 |
| `strategy_statistics` | 比较注册策略的样本量、完整度、时间范围和风险指标 |
...
每日收盘 Top10 作为独立的 `close_baseline` 研究批次保存，普通日更不能覆盖。下一交易日 09:00 至 09:30 前的合法终选可以接替活动批次；原收盘快照及逐股记录先完整归档，归档、批次切换和终选写入同一事务。现行前向评价只接受 `premarket_final`，旧版收盘、收盘基线和历史补算按阶段及评分版本分别研究。`live` 兼容标签本身不代表前向资格，详见[预测时间契约](docs/Prediction_Time_Contract.md)。
...
推荐情报刷新链路现为 relay-only，只服务一进二盘后新增情报的受控更新，不再混入其他候选结构。每日收盘主流水线会为七项注册策略生成或读取当日不可变快照，并回填已经成熟的 D+1、D+3、D+5 Outcome；探索策略保持无名次观察池。
...
预测来源有严格时间口径：同日 15:30 后保存收盘基线并生成动态草稿，下一交易日 09:00 起、09:30 开盘前固化最终 Top10。公共写入层校验阶段、实际生成时间、信息截止和交易日历；直接脚本补算过去日期只能写入 `historical_backtest`。终选不会被普通刷新覆盖，处理跨过开盘则回滚，错过窗口后进入 `missed_cutoff`。历史异常使用独立审计标记排除，保留原件；复盘页面可按阶段与评分版本分别查看。
