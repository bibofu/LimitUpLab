...
每日收盘 Top10 基线以整批快照写入 `agent_live_prediction_snapshots`。同一交易日首次写入后不可被重复日更或新评分版本覆盖；Agent、Review、Evaluation 和质量审计用它保持前向审计口径，历史重算结果不能混入真实样本。盘前推荐与涨停池首板页共同读取最新版本化动态榜；新增公告、龙虎榜或可验证的人气变化可以让基础 Top10 外候选换入，但不会覆盖收盘基线。
...
预测来源有严格时间口径：收盘后生成可变草稿，下一交易日 09:00 起、09:30 开盘前固化的最终 Top10 标记为 `live`；事后补算只标记为 `historical_backtest`，不能混入真实前向预测准确率统计。已固化结果不会被普通刷新覆盖；错过开盘前固化窗口时只记录 `missed_cutoff`，不允许用开盘后数据伪装盘前预测。
