...
`v1.0.0` 是首个可用基线，定义为基于完整收盘数据的首板复盘与次日一进二候选研究工具。`v1.1.0` 在此基础上增加双策略盘前研究：前一交易日收盘结果形成首板挖掘和一进二接力 Top30 草稿，新闻与财报持续重排，交易日 09:25:10 再结合集合竞价终值生成两类策略当天唯一正式 Top10，复盘只读取正式版本。详见 [V1.1 阶段里程碑](docs/V1.1_Milestone.md)。
...
- 首板挖掘与一进二接力的“收盘基础池 + 动态新闻财报 + 09:25 唯一正式预测”链路
...
4. V1.0 历史日期保存收盘 Top10；V1.1 启用日起只生成可覆盖的盘前候选草稿。
5. 回填近期 Top10 的 D+1 至 D+5 走势。
6. 输出 JSON 数据健康报告。

盘前推荐中的两类 Top30 候选共用动态草稿流水线。它每 30 分钟批量更新
最新行情和最近个股新闻，并读取同花顺最新季度利润表；财报缓存 24 小时，
避免对不会频繁变化的数据重复请求。收盘基础分保持可追溯，新闻与财报以
有边界的修正更新草稿分和名次；草稿不计入回测，09:25:10 集合竞价终选
Top10 才是当天正式预测：
...
任务默认在每个工作日 `16:10` 执行，并启用错过后补跑、失败重试和禁止并发执行。一次执行会自动完成交易日判断、原始数据同步、首板特征与 enrichment、盘前候选草稿、历史 Top10 的 D+1 至 D+5 K 线缓存、Outcome 回填、每日复盘快照和健康检查。V1.1 启用日起，收盘任务不再把草稿标记为 `live`；次日 `09:25:10` 的竞价终选才保存正式预测并进入后续复盘。

预测来源有严格时间口径：V1 历史日期沿用收盘后 `live` 口径；从 `2026-08-31` 基础交易日起，只有次日集合竞价闭合后生成的 Top10 会标记为正式 `live`，收盘草稿和事后补算的 `historical_backtest` 都不能进入真实预测准确率统计。
