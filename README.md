...
盘前推荐只保留一进二接力链路，以当日全部合格首板为候选池并动态展示 Top10。流水线每
30 分钟批量更新最新行情、个股新闻、龙虎榜和当前可用人气榜，并读取同花顺最新季度利润表；
财报缓存 24 小时，避免对不会频繁变化的数据重复请求。收盘基础分保持可
追溯，盘后新增公告、新上龙虎榜和可比较的人气变化以有边界的修正更新研究
分与名次；动态一进二 Top10 允许换入换出，盘前推荐与首板页前 10 名读取同
一份当前版本。系统只保存当前草稿和字段变化记录，不堆积半小时全量快照；目标
交易日 09:00 起完成最后一次刷新，随后将 Top10 固化为不可变最终预测，
...
任务默认分为每个工作日北京时间两个阶段：`15:30` 提前采集展示，`16:10` 补齐核验。提前阶段只同步原始数据、首板特征和近七交易日观察池 K 线，不写入预测或复盘快照；页面标注“待补齐核验”。核验阶段独立重采当日数据、刷新 enrichment，检查来源新鲜度、涨停数量交叉校验和候选关键输入，再固化收盘 Top10，并继续历史 D+1 至 D+5 K 线、Outcome 回填与复盘检查。核验缺口明确记为 partial 并重试，不保证上游数据在 16:10 已全部发布；现存不可变预测不会被覆盖。

Windows 安装脚本会注册 `LimitUpLab-DailyCloseLoop-Preview` 和 `LimitUpLab-DailyCloseLoop` 两个任务。两阶段分别记录成功状态，共用互斥锁；核验任务等待提前任务结束，锁等待超时返回非零以便重试。错过提前窗口（16:10 以后）不再补跑 preview，由 final 负责采集。CLI 可使用 `--phase preview` / `--phase final`，默认 final；当天 final 不早于 16:10 执行。节假日跳过，历史补数仍只能写入历史来源。

预测来源有严格时间口径：自动任务在同日 16:10 补齐核验通过后保存收盘基线（底层来源校验仍要求不早于 15:30），随后生成动态草稿，下一交易日 09:00 起、09:30 开盘前固化最终 Top10。公共写入层校验阶段、实际生成时间、信息截止和交易日历；直接脚本补算过去日期只能写入 `historical_backtest`。终选不会被普通刷新覆盖，处理跨过开盘则回滚，错过窗口后进入 `missed_cutoff`。历史异常使用独立审计标记排除，保留原件；复盘页面可按阶段与评分版本分别查看。
