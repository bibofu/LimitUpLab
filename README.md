...
`v1.0.0` 是首个可用基线，定义为基于完整收盘数据的首板复盘与次日一进二候选研究工具。`v1.1.0` 记录了双策略盘前研究和集合竞价实验。当前主线重新收敛到可审计的涨停后研究：集合竞价和面向未涨停股票的首板挖掘均已退出运行链路，每天收盘后固化一进二 Top10 并进入复盘。历史实验仍由 Git 标签和阶段里程碑保留。
