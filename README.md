...
交易日 09:00 完成最后一次刷新，随后将 Top15 / Top10 固化为不可变最终预测，
并以最终一进二 Top10 替换复盘基线：
...
预测来源有严格时间口径：收盘后生成可变草稿，下一交易日 09:00 固化的最终 Top10 标记为 `live`；事后补算只标记为 `historical_backtest`，不能混入真实前向预测准确率统计。09:00 后同一基准日的普通刷新不会覆盖最终预测。
