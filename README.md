...
`v1.0.0` 是首个可用基线，定义为基于完整收盘数据的首板复盘与次日一进二候选研究工具。`v1.1.0` 记录了双策略盘前研究和集合竞价实验。`v1.2.0` 重新收敛到可审计的收盘后研究主线：移除集合竞价运行链路，每天收盘后固化一进二 Top10 并进入复盘；另一条研究链路改为“低位挖掘”，从热门题材与最新催化出发，结合财报和 60 日 K 线筛选低位启动观察池，不再输出“次日首板 Top10”。本版本同时完成原生 Function Calling、滚动 Session Memory、收盘信息截止契约和个股 K 线加载优化。版本记录见 [V1.2 阶段里程碑](docs/V1.2_Milestone.md)，集合竞价实验仍由 [V1.1 阶段里程碑](docs/V1.1_Milestone.md) 和 `v1.1.0` 标签保留。
...
- 盘前推荐页新增涨停后缩量整理观察池，支持固定条件筛选、历史日期查看和缺失原因解释；独立于一进二前向统计，见 [第一版说明](docs/Consolidation_Strategy_V1.md)
...
盘前推荐中的两类候选共用动态草稿流水线：低位挖掘在收盘后固定 50 只候选池并
动态展示 Top15，一进二以当日全部合格首板为候选池并动态展示 Top10。流水线每
...
交易日 09:00 起完成最后一次刷新，随后将 Top15 / Top10 固化为不可变最终预测，
