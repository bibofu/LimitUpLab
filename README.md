...
`v1.0.0` 是首个可用基线，定义为基于完整收盘数据的首板复盘与次日一进二候选研究工具。当前开发分支已开始 V2 增量：盘前推荐在保留前一晚不可变初选的基础上，于交易日 09:25:10 使用集合竞价终值生成独立终选快照；该能力不回写或覆盖 V1 预测记录。
...
- 首板挖掘与一进二接力的“收盘初选 + 09:25 竞价终选”双快照链路
